...
The same availability list includes `ultra` for models that support it, including Astra. The canonical Luna roles remain at `max` because their model catalog supports `max` but not `ultra`.
